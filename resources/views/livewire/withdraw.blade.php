                ...
                @forelse($withdraws as $withdraw)
                    <tr>
                        ...
                        <td>
                            <p class="mt-1 text-primary fs-6 text-break text-nowrap"><i class="fas fa-coins"></i>
                                Amount: {{ $withdraw->amount }} NSP</p>
                            <p class="text-danger"><i class="fas fa-coins"></i> Final: {{ $withdraw->finalAmount }} NSP
                            </p>
                            <p class="mb-1 text-success"><i class="fas fa-coins"></i> Discount:
                                {{ $withdraw->discountAmount }} NSP</p>
                        </td>

                        <td class="d-none d-md-table-cell">
                            <p class="mb-1">{{ $withdraw->status }}</p>
                        ...
                        <td class="d-none d-md-table-cell">
                            @if ($withdraw->status == 'requested')
                                <div class="text-center">
                                    <a href="{{ route('withdraws.completeOrder', $withdraw) }}"
                                        class="btn btn-success"><i class="fas fa-check text-light"></i></a>
                                    <button class="btn btn-warning" onclick="openModal('reject',{{$withdraw->id}})"><i class="far fa-times-circle"></i></button>
                                </div>
                            @endif
                            <div class="text-center mt-2">
                                <a href="{{ route('withdraws.edit', $withdraw) }}" class="btn btn-primary"><i
                    ...
                    <tr class="d-table-row d-md-none">
                        <td colspan="2">
                            ...
                            badge text-bg-success @endif">
                                <i class="fas fa-info-circle me-1"></i>{{ $withdraw->status }}
                            </div>
                            @switch($withdraw->method)
                                @case('سيريتل')
                    ...
                    <tr class="bg-dark d-table-row d-md-none">
                        <td colspan="5" class="bg-body-secondary">
                            @if ($withdraw->status == 'requested')
                                <a href="{{ route('withdraws.completeOrder', $withdraw) }}"
                                    class="btn btn-success btn-sm"><i class="fas fa-check text-light"></i></a>
